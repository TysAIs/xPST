# xPST v1.0.0 — Final Gap Analysis (archived)

> **Archived point-in-time audit** (moved from the repo root to
> `docs/history/` — links below are relative to the repo root).
...
> [docs/INSTALL.md](../INSTALL.md#capability-truth-table) is authoritative today.
...
The single most common defect is **hardcoded narrow platform tuples** instead of iterating a shared constant. The same `("youtube", "instagram", "x")` or `(..., "tiktok")` literal is copy-pasted into ~10 locations. Each is an independent place where Threads silently falls out. Fixing the root cause = define one authoritative platform list and drive every loop/choice/validator from it.
...
|---|---------|----------|----------|
| 1 | `xpst connect threads` rejected by CLI | **HIGH (BUG)** | `cli.py:869` |
| 2 | SchedulePage UI cannot schedule Threads | **HIGH (GAP)** | `SchedulePage.qml:20,66` |
| 3 | README env-var reference is fabricated (`__` scheme, `XPST_RATE_LIMITS__*`, `XPST_FFMPEG_PATH`) | **HIGH (BUG)** | README §128,§657 |
| 4 | Full-pipeline integration tests cover only 3 of 5 platforms | **HIGH (GAP)** | `tests/test_integration.py:122` |
...
| 8 | `get_statistics` drops Threads from per-platform counts | **MEDIUM (BUG)** | `state_manager.py:388` |
| 9 | `xpst auth` + auth-status omit Threads | MEDIUM | `cli.py:904,945` |
| 10 | `xpst health` only reports *enabled* platforms (disabled platforms vanish) | MEDIUM | `engine.py:1086` |
| 11 | Threads config YAML keys (`client_id/secret`) ignored by loader | MEDIUM | `config.py:543` |
| 12 | backend preview/config-load/validation loops drop Threads | MEDIUM | `backend.py:719,1097,1959,1993` |
| 13 | TikTok/Threads analytics are best-effort yt-dlp; usually empty, shown as real `0` | MEDIUM | `analytics.py:367-490`; `backend.py:529` |
...
| 17 | Video encoding: TikTok/Threads use IG profile; mapping duplicated 3× | MEDIUM | `video.py:199,266`; `upload_service.py:622` |
...
**Mostly PASS.** Both `PlatformRegistry` (`platforms/base.py:269`) and `SourceRegistry` (`sources/base.py:231`) use auto-discovery, so uploaders/sources self-register. Layers 1–7 (uploaders, engine registry, `connect.py` all_platforms, analytics collectors, dashboard, backend `PLATFORMS` tuple) are clean for all 5. Threads having no `sources/` module is **intentional** — it is destination-only (`platforms/threads.py:58` declares `roles=(ProviderRole.DESTINATION,)`).

### GAP-1 — SchedulePage cannot schedule Threads — **HIGH**
`src/xpst/desktop_app/qml/pages/SchedulePage.qml:20` — `formPlatforms: ({ youtube: true, instagram: false, x: false, tiktok: false })`; push logic at `:66-69` only emits those 4. Threads has a full uploader and engine support but **cannot be selected for a scheduled post in the UI**.
**Fix:** add `threads` to `formPlatforms` + checkbox; ideally render the form from a shared platform list.

### GAP-2 — `previewPost` default targets omit threads/tiktok — **HIGH**
`src/xpst/desktop_app/backend.py:719-724` — `enabled_platforms` built from `("youtube","instagram","x")` only; `targets = requested_platforms or enabled_platforms`. Explicit selection works for all 5, but a no-arg preview silently excludes enabled Threads/TikTok from preflight.
...
- `backend.py:1097` config-load enable toggles iterate `("youtube","instagram","x")` → imported TikTok/Threads enable state dropped. (MEDIUM)
- `backend.py:1959` (encoding validation) and `:1993` (rate-limit bounds) skip Threads → invalid values unvalidated; `rate_limits` serialization at `:614` omits Threads so its limits never reach the Settings UI. (MEDIUM)
- `backend.py:871` progress-emit loop omits Threads → no initial progress tick (cosmetic). (LOW)
**Fix:** drive all four from `PLATFORMS`.

### GAP-6 — ConnectPage static fallback omits Threads — **LOW**
`ConnectPage.qml:51-55` `fallbackDestinations()` lists only 3. Mitigated at runtime (`:133` overrides via `controller.getProviders()`), but if that call fails Threads becomes unconnectable. Add it to the fallback for graceful degradation.

---

## AREA 2 — Video Encoding

**PASS with caveats.** `encode_for_platform()` dispatch (`utils/video.py:260-270`) reaches a valid branch for all 5; unknown platforms `raise ValueError` (no silent crash). Uploads route through encoding at `services/upload_service.py:259` → `_encode_for_platform` → `:651`, with passthrough, compliance-skip (fidelity invariant), caching, and partial-output cleanup.

### GAP — TikTok/Threads use the Instagram profile, mapping duplicated — **MEDIUM**
`video.py:266-268` maps these 2 to `_build_instagram_cmd` (1080×1920 portrait, CRF 20, GOP 72). The fallback is **explicit/commented, not a silent `else`** — acceptable for v1.0. But the same "which profile does platform X use" mapping logic is copy-pasted at `video.py:199`, `video.py:266`, and `upload_service.py:622` — three copies that can drift.
**Fix:** factor the mapping into one helper.
...
**PASS on wiring.** The compat wrapper is sound: `state.py:32-34` constructs one `NewStateManager` and reuses *its* store (`self._store = self._new_manager._store`) to avoid a dual-lock race. The production engine imports the wrapper (`engine.py:37`). `state_store.py:247-275` seeds `health.platforms` with all 5; `posted_videos`/`content_hashes` are platform-agnostic; schema v2 with v1→v2 migration. `mark_video_posted` is fully platform-agnostic (`state.py:150-170`).

### BUG — `get_statistics` drops Threads — **MEDIUM**
`state_manager.py:388` — `by_platform = {"youtube":0,"x":0,"instagram":0,"tiktok":0}`. Threads posts are counted in `cross_posted_count` (`:397`) but the `if platform in by_platform` guard (`:395`) silently drops them, so per-platform stats under-report them as **zero**. Confirmed by direct read.
**Fix:** seed `by_platform` from the 5 health-platform keys.
...
**PASS on plumbing.** `analytics.py:179-190` dispatches `_collect_*` for all 5; `_discover_post_ids` seeds all 5 (`:508-515`). `analytics_store.py:29-54` schema is platform-agnostic (generic metric columns + `extra` JSON). `backend.py:_refresh_recent_posts` (`:487-592`) iterates `posted_to.items()` and enriches every platform with no allowlist.

### GAP — fidelity is 3 real + 3 best-effort, presented identically — **MEDIUM**
`analytics.py:367-490` — TikTok/Threads all fall back to generic `yt-dlp` URL scraping; the Threads handler's own docstring admits "no stable public metrics API," so it frequently returns empty. `backend.py:529-532` then defaults `views/likes/comments/shares = 0`, so the UI shows **real-looking zeros** for platforms that simply have no collector.
...
**PASS.** All 5 platform config dataclasses present (`config.py:185,200,209,225,240`), all wired into `XPSTConfig`, `DEFAULT_CONFIG`, merge/env/save/rate_limits. Instagram defaults to `auth_mode="graph_api"` (`config.py:220,56`) — correct.

### GAP — Threads YAML keys ignored — **MEDIUM**
On-disk config stores `client_id`/`client_secret` for Threads, but the dataclass expects `graph_access_token`/`threads_user_id` (`config.py:240`); `_merge_config` (`:543-558`) reads only dataclass-named keys, so the YAML values are silently dropped.
...
`xpst --help` (38 commands), `--version` (1.0.0), `post/analytics/schedule/auth --help` all exit 0.

### BUG — `xpst connect threads` rejected — **HIGH**
`cli.py:869` — `type=click.Choice(["tiktok","youtube","x","instagram"])`. Confirmed live: `xpst connect threads` → `Error: ... 'threads' is not one of ...`. Yet `connect.py:858-874` fully supports all 5 (`connect_threads` defined). The Click Choice blocks individual connection of the newest platform.
**Fix:** add `threads` to the Choice (derive from a shared constant).

### GAP — `xpst auth` restricted to 4 platforms — **MEDIUM**
`cli.py:904` `valid_platforms = {"tiktok","youtube","x","instagram"}`; dispatch (`:915-922`) has no threads branch; `_show_auth_status` (`:945`) reports only youtube/x/instagram.

### GAP — `xpst health --json` omits disabled platforms — **MEDIUM**
`engine.check_health` (`engine.py:1086`) iterates only `self._platforms`, populated only for `enabled` platforms (`:297-342`). On the test machine Threads was `enabled: false`, so it vanished from health entirely. Misleads users into thinking the platform doesn't exist.
...
Dedicated per-platform error matrices exist for the **newest** two: `TestTikTokUploader`/`TestThreadsUploader` in `tests/test_phase_a_platforms.py` (success / no-token / auth-expired / rate-limit / network-error / health / followers / delete).

### GAP — full-pipeline integration covers only 3 of 5 — **HIGH**
`tests/test_integration.py:122` `test_full_pipeline_tiktok_to_all` exercises real `engine.check_and_post()` but only over youtube/x/instagram (hardcoded `:156,163`); docstring says "all platforms." `TestCircuitBreaker`/`TestRateLimit`/`TestHealthCheck` likewise assert over the same 3. Grep: threads=1 in `test_integration.py`. **Threads is never validated through the real orchestration path.**
**Fix:** parametrize the integration target set over all 5.

### GAP — no per-platform test files; oldest 3 lack focused upload matrix — **MEDIUM**
No `test_<platform>.py` convention. youtube/x/instagram have no equivalent of the Phase-A error matrix. Threads is weakest overall (3 files, 0 integration). Add a parametrized `test_platforms.py` running the same matrix across all 5.

---

## AREA 9 — Error Handling & Resilience

**PASS with one architectural risk.** Every uploader converts auth failures into a typed `UploadResult(success=False, error="<CODE>")` rather than raising (threads.py:157, tiktok.py:161, x.py:140, instagram.py:599, youtube.py:242). Circuit breakers are applied centrally in `upload_service.py` (`:176` gate, `:339/387/413` record), keyed by arbitrary platform name → covers all 5 uniformly.
...
*LOW:* circuit-breaker behavior is only tested for youtube; no breaker test for threads.

---

## AREA 10 — Documentation Accuracy

Headline numbers verified accurate: 38 CLI commands, 28 MCP tools, 5 platforms, 1555 tests (2 skipped). The weak spots:
...
1. **One-line, high-impact:** add `threads` to `cli.py:869` Choice (Finding 1).
...
4. **Docs:** fix the env-var reference and Docker status (Findings 3, 6).
5. **Tests:** parametrize integration tests over all 5 platforms (Finding 4) — this would have caught most of the above.
