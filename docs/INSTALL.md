...
| TikTok as a destination | **Draft mode (implemented; not live-verified)** | With OAuth credentials (`video.upload` scope), uploads land in your TikTok inbox as drafts you finish in the app (`draft_mode: auto`/`always`; PR #266). Public Direct Post awaits TikTok's developer-app audit. Drafts are reported PENDING, never as published posts. |
