...
> **Complete guide** to using xPST's 37 MCP tools with AI agents. Drive the entire cross-posting workflow from Claude Desktop, Cursor, or any MCP-compatible client.
...
The **Model Context Protocol** (MCP) is an open standard that lets AI assistants interact with external tools and data sources. xPST exposes 37 MCP tools that let any MCP-compatible AI agent:
...
xPST exposes 37 MCP tools across five groups: core operations, analytics & insights, content & knowledge, configuration & scheduling, and the knowledge base. All posting destinations — YouTube, Instagram, X/Twitter, TikTok, and Threads — are valid `platforms` values.
