...
|---|---|---|---|---|---|
| **xPST** | **Free, MIT OR Apache-2.0** | **Yes** | YT, X, IG, TikTok, Threads, Messenger | **Official OAuth** (ban-safe) | **CLI + MCP (28 tools) + FastAPI dashboard + desktop (optional)** |
...
2. **AI-agent-native.** 28 MCP tools (`xpst_post`, `xpst_suggest_caption`, `xpst_health`, `xpst_analytics`, scheduling, Messenger auto-reply, KB, transcripts, search). **No competitor ships an MCP server** — this is the "for AI agents" moat.
...
6. **1536 tests, enterprise-hardened** — thread-safe state, circuit breakers, anti-bot jitter, rate-limit calendars, MCP security hardening.
