...
def _flatten_route_paths(routes: list) -> list[str | None]:
    """Route-table paths, descending into included routers.

    FastAPI <0.141 copies an included APIRouter's routes into ``app.routes``;
    FastAPI >=0.141 (starlette 1.6) keeps ONE composite entry (``_IncludedRouter``)
    with no ``.path`` and the real routes reachable via ``original_router.routes``.
    Counting without descending would under-report on new FastAPI and make the
    published count depend on the runner's dependency versions.
    Mounts and ordinary routes carry a ``.path`` and count as one entry each,
    matching the historical route-table semantics.
    """
    paths: list[str | None] = []
    for route in routes:
        ...
        if nested:
            paths.extend(_flatten_route_paths(nested))
        else:
            paths.append(None)
    return paths
...
def http_route_counts() -> tuple[int, int]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="xpst-counts-") as config_dir:
            ...
            paths = _flatten_route_paths(app.routes)
            total = len(paths)
