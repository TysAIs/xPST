...
def generate_release_evidence(dist_dir: Path, output_dir: Path, output: Path, version: str, checks_run: bool, lane: str | None = None) -> None:
    """Generate machine-readable release evidence for shipped artifacts."""
    files = find_dist_files(dist_dir)
    lane_prefix = f"{lane}-" if lane else ""
    generated_files = [
        f"{lane_prefix}SHA256SUMS",
        f"{lane_prefix}SHA512SUMS",
        f"{lane_prefix}pypi.json",
        f"{lane_prefix}xpst-sbom.cdx.json",
...
def main() -> int:
    ...
    parser.add_argument(
        "--lane",
        default=None,
        help="Lane name (e.g. python, windows, linux, macos). When set, "
        "metadata files are prefixed with the lane name to avoid "
        "collisions when merged in the release job.",
    )
    ...
    # Lane prefix for metadata files to avoid merge collisions
    lane_prefix = f"{args.lane}-" if args.lane else ""

    # Generate checksums
    sha256_file = output_dir / f"{lane_prefix}SHA256SUMS"
    sha512_file = output_dir / f"{lane_prefix}SHA512SUMS"
    generate_checksum_files(dist_dir, sha256_file, sha512_file)

    # Generate PyPI JSON
    pypi_json = output_dir / f"{lane_prefix}pypi.json"
    generate_pypi_json(dist_dir, pypi_json, version)

    # Generate SBOM
    sbom_file = output_dir / f"{lane_prefix}xpst-sbom.cdx.json"
    generate_sbom(dist_dir, sbom_file, version)

    # Generate release notes
    release_notes = output_dir / f"{lane_prefix}RELEASE_NOTES.md"
    ...
    evidence_file = output_dir / f"{lane_prefix}RELEASE_EVIDENCE.json"
    generate_release_evidence(dist_dir, output_dir, evidence_file, version, checks_run=not args.skip_checks, lane=args.lane)
