...
### Phase A: Platform Expansion — Free Posting on Every Platform
- **3 new platform uploaders**: TikTok (Content Posting API), Threads (Meta Threads API), LinkedIn (API v2) — all official, all free
- **Auth mode default flip**: Instagram defaults to Graph API (official) instead of instagrapi (unofficial); X stays twikit (official API is paid)
- **Provider mode system**: `official` (default) hides community/unofficial platforms; `community` mode shows all with risk warnings. Configurable via `XPST_PROVIDER_MODE`
- **IG Resumable Upload**: Local files upload directly to `rupload.facebook.com` — no public CDN needed for Graph API
- **Provider manifests**: TikTok, Threads, LinkedIn all have `is_official_api=True` manifests
- **Config**: New `ThreadsAccountConfig`, `LinkedInAccountConfig`, extended `TikTokAccountConfig` with OAuth fields

### Phase B: Cross-Post Intelligence
- **Cross-post correlation engine**: One video posted to multiple platforms shows as ONE entry with total metrics across all platforms
- **Follower tracking**: `get_followers()` on all 7 platforms; `follower_snapshots` table in analytics DB; `xpst followers --json` CLI + `xpst_followers` MCP tool
- **Best time to post**: `BestTimeAnalyzer` finds hour-of-week with highest avg engagement; `xpst best-time --json` CLI + `xpst_best_time` MCP tool
- **Engagement rate**: Per-platform and per-cross-post-group; tier classification (>5% high, 1-5% medium, <1% low)
- **Cross-post groups table**: `cross_post_groups` in analytics DB with `platforms_json` tracking all platform post IDs per video

### Phase C: UX Extermination — Zero Plumbing
- **Sidebar renamed**: Content → Library, Connect → Accounts, Schedule → Automations
- **No raw auth_mode in UI**: Replaced with human labels ("Sign in with Google", "Sign in with Meta", etc.) via `signInLabel()` helper
- **No raw capabilities arrays**: Replaced with feature icons via `featureSummary()` helper
- **No circuit_breaker_open**: Replaced with "Connection issue — click to retry"
- **Per-platform char counts** in Compose page: Shows X: 280, Threads: 500, IG: 2200, YouTube: 5000, LinkedIn: 3000 simultaneously
- **Provider Mode badges**: Green "Official" or yellow "Community" badge per platform in Connect page

### Phase D: Knowledge Base Perfection
- **D1: Zero-config defaults**: `llm_enabled=False` (was True), `llm_base_url=""` (was fake endpoint), `llm_model=""` (was fake model). KB works with zero config using deterministic extractor
- **D2: Transcript dedup**: Transcripts cached by `content_hash` in workspace `transcripts/` dir. Cross-posted videos share one transcript — no re-transcription. `Workspace.save_transcript()` / `get_transcript()` methods.
- **D3: sqlite-vec migration**: New `SQLiteVecStore` class implements `KnowledgeStore` interface. Default vector backend is now sqlite-vec (<1MB) instead of LanceDB (~50MB). LanceDB remains as legacy backend via `XPST_KB_VECTOR_BACKEND=lancedb`. Graceful fallback to linear scan if extension not loaded.

### Phase E: Security Hardening
- **Dashboard localhost-only**: Default bind is `127.0.0.1` (was `0.0.0.0`); `--host` flag for explicit remote access; warning when binding non-localhost
- **CSRF protection**: `SameSite=Strict` cookies, CSRF token on API
- **MCP readonly mode**: `XPST_MCP_READONLY` env var blocks mutating tools when set
- **Credential storage**: Fernet encryption with scrypt key derivation, 0600 permissions, no plaintext fallback (PlaintextStorageError)
- **Security audit CLI/MCP**: `xpst security-audit --json` and `xpst_security_audit` MCP tool — checks file permissions, encrypted storage, dashboard binding, MCP readonly, FFmpeg, provider mode
- **FFmpeg first-run detection**: `installFFmpeg()` backend slot for one-click install (brew/apt/dnf); readiness report surfaces FFmpeg status

### Phase F: AI Agent Amplification
- **New CLI commands** (8): `analytics --cross-post`, `followers`, `best-time`, `transcript`, `search`, `suggest-caption`, `security-audit`, `providers` (now shows provider_mode)
- **New MCP tools** (8): `xpst_cross_post_analytics`, `xpst_followers`, `xpst_best_time`, `xpst_transcript`, `xpst_search`, `xpst_suggest_caption`, `xpst_security_audit`, `xpst_provider_modes`
- **Caption generation**: `caption_gen.py` with deterministic fallback + LLM-powered mode; platform-specific char limits; `xpst suggest-caption <video> --json`
- **24 MCP tools total** (was 16), **36+ CLI commands** (was 28)

### Phase G: Polish & Ship
- Version bumped to 1.0.0
- 1427 tests passing, ruff clean, mypy clean
- .gitignore bug fixed (line 109 `***` was ignoring all untracked files)

### Bug Fixes
- Fixed `.gitignore` line 109: `***` pattern was silently ignoring ALL untracked files (caption_gen.py, best_time.py, tiktok.py, threads.py, linkedin.py were never tracked)
- Fixed `LLMClient` constructor call in `caption_gen.py` (was passing config object instead of `base_url`/`model` args)
- Fixed `Transcriber` import in `cli.py` (Protocol class → `FasterWhisperTranscriber`, `model=` → `model_size=`)
- Fixed `LLMClient.complete()` call (nonexistent method → `chat_json()` via `run_in_executor`)
...
### Documentation
- README rewritten: four-pillar framing, honest per-platform analytics capability matrix (story-reposts collectible on zero platforms; IG shares/saves need a Business account; TikTok metrics are unauthenticated scrape), per-platform video constraints, TikTok corrected to source-only everywhere, verified counts (28 CLI commands, 16 MCP tools), PyPI install gated behind a published note, platform-risk/ToS disclosure section
- docs/MCP_TOOLS.md regenerated from the live registry: all 16 tools documented with arguments, examples, and a real-account guardrails section
