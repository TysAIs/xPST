...
def test_release_surfaces_are_all_110() -> None:
    ...
    assert versions == {name: "1.2.1" for name in versions}
...
def test_release_asset_assertion_runs_and_gates(tmp_path: Path) -> None:
    ...
    complete = {
        "mac/dmg/xPST_1.2.1_aarch64.dmg": "dmg",
        "mac/dmg/media-binaries-PROVENANCE-aarch64-apple-darwin.txt": "prov",
        "win/nsis/xPST_1.2.1_x64-setup.exe": "exe",
        "linux/appimage/xPST_1.2.1_amd64.AppImage": "appimage",
    }
    proc = run(complete)
...
def test_release_asset_assertion_requires_the_provenance_record(tmp_path: Path) -> None:
    ...
    for name in ("xPST_1.2.1_aarch64.dmg", "xPST_1.2.1_x64-setup.exe", "xPST_1.2.1_amd64.AppImage"):
