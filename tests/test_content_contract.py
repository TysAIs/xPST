...
EXPECTED_MATRIX: dict[tuple[str, str], bool] = {
    ...
    ("instagram", "image"): False,  # no feed-photo path; the manifest no longer declares it
    ...
    ("threads", "text"): False,  # only media_type VIDEO exists; the manifest no longer declares it
    ("threads", "thread"): False,
}

#: Declared-but-unimplemented: an agent reading the manifest would try these.
#: A destination that has no path for a content type must not advertise it: the
#: manifests for Instagram (``image``) and Threads (``text``) dropped their
#: unimplementable labels, so X's ``thread`` is the remaining case where a
#: provider-level capability is declared and the engine has no path for it.
...
def test_capability_matrix_is_json_serializable() -> None:
    ...
    assert payload["platforms"]["x"]["declared_but_unimplemented"] == ["thread"]
