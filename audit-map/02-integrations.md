...
| `accounts.youtube.client_secrets` | `credentials/youtube_client_secrets.json` | Google OAuth client (production app `xpst-prod-87871`, client `88230346195-…`) | WORKING — health live: `authenticated: true, session_valid: true`, channel **“Ty's AI's”** `UCvaWo7MzVE1vu9d7MA6vzgA` |
...
| `accounts.youtube.channel_id` | `UCvaWo7MzVE1vu9d7MA6vzgA` | Destination channel (Ty's AI's / JaceDigital brand) | WORKING; **replaced** the earlier `UCRF2kQGQ6DJiWR4BbY4SPqA` “gumbo racing” channel (see §5) |
...
| `tiktok_dev_account.txt` | TikTok Developer Portal | Plaintext metadata: `EMAIL: tys.aissss@gmail.com`, `PASSWORD: TkDev-Tys2026!` | VALID aid (not read by xPST runtime); **plaintext credential note** |
...
|---|---|---|
| YouTube | `xpst health` | **PASS** — authenticated, session valid, channel “Ty's AI's” `UCvaWo7MzVE1vu9d7MA6vzgA` |
...
2. **Old YouTube channel “gumbo racing” `UCRF2kQGQ6DJiWR4BbY4SPqA`.** Replaced by “Ty's AI's” `UCvaWo7MzVE1vu9d7MA6vzgA` (config + live health). No occurrence left anywhere under `~/.xpst` or repo docs/README/configs (grep = 0) — but **still stale inside** bot `memories/OPERATING.md` (“channel 'gumbo racing.'”) and the xpst-cross-posting skill body. → Update bot memory/skill.
...
7. **Credential cross-refs:** `tiktok_dev_account.txt` contains plaintext TikTok dev-portal password (`TkDev-Tys2026!`) and the live email `tys.aissss@gmail.com`; `youtube_token.json` duplicates the client_id/secret (normal Google pattern). No dead emails/keys found in config; no 404 URLs found.
