...
DESTINATION_CONTENT_PROFILES: dict[str, DestinationContentProfile] = {
    ...
    "instagram": _publish_profile(
        "instagram",
        "Instagram Reels",
        # No `image` label: both container calls hard-code media_type=REELS, so
        # there is no single-image publish path. The manifest stopped declaring
        # it (the truth fix), and this table mirrors the manifests.
    ...
    "threads": _publish_profile(
        "threads",
        "Threads",
        # No `text` label: the container call hard-codes media_type=VIDEO, so
        # there is no text path. The manifest stopped declaring it (the truth
        # fix), and this table mirrors the manifests.
        declared_labels=("video",),
        implemented=(ContentType.VIDEO,),
        notes={
            ContentType.VIDEO: (
