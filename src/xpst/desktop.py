...
def launch_desktop_app(
    ...
    """Launch the native xPST desktop application.

    Preference order:
      1. PySide6/QML native desktop app.
      2. NiceGUI/webview fallback when PySide6 is unavailable.

    Platform integrations (dock icon, taskbar shortcut, desktop entry)
    are still installed/updated on launch.
    """
    try:
    ...
    try:
        ...
        logger.error("Native desktop app failed: %s", exc)
        if "PySide6" in str(exc) or "pyside6" in str(exc).lower():
            return _launch_webview_desktop(config_dir=config_dir, port=port)
        raise


def _launch_webview_desktop(
    config_dir: str = "~/.xpst",
    port: int | None = None,
) -> None:
    """Webview fallback for environments without PySide6/QML."""
    try:
    ...
    logger.info("Starting xPST webview dashboard on %s", url)
...
def launch_browser_fallback(
    ...
    start_dashboard(port=port, host="0.0.0.0", config_dir=config_dir)  # nosec B104 - local dashboard, intentional bind
