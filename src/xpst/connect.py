...
def connect_linkedin(config: XPSTConfig) -> bool:
    ...
    console.print("  2. Create or select an app and request the 'Share on LinkedIn' product")
    console.print("  3. Generate an access token with the w_member_social scope")
    console.print("  4. Copy your member URN (urn:li:person:XXXX) or plain ID\n")
    ...
    linkedin_user_id = console.input("[cyan]LinkedIn user URN or ID: [/cyan]").strip()
    if not linkedin_user_id:
        console.print("[red]❌ LinkedIn user ID required.[/red]")
        return False

    access_token = _input_secret("Access token: ")
    if not access_token:
        console.print("[red]❌ Access token required.[/red]")
        return False
    ...
    console.print("[green]✅ LinkedIn configured![/green]")
    return True
