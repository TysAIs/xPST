...
def _run_payload(results: list[CrossPostResult]) -> dict[str, Any]:
    ...
    G28: agents need the per-video outcomes and post URLs, not a bare success
    string. ``ok`` used to be hard-coded ``True``, so an agent that trusted it
    reported success for a failed run. It now means what it says, derived from
    the same per-platform ``UploadResult.is_published`` the engine's own status
    flags use: every upload in every result must be provably published, and
    "nothing ran" is not success.

    The counts are explicit so a caller can tell "nothing to do" apart from
    "everything failed": ``attempted``/``uploads``/``published`` come from the
    per-platform upload rows, ``processed``/``succeeded``/``failed`` from the
    per-video results.
    """
