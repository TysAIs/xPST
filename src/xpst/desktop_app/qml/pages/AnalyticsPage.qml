...
Page {
    ...
    property var analyticsJson: {
        ...
        } catch(e) { console.error('AnalyticsPage: failed to parse analyticsData', e); if (typeof showToast !== "undefined") showToast("Could not load analytics data", true) }
    ...
    Connections {
        ...
        function onDataChanged() {
            ...
            } catch(e) { console.error('AnalyticsPage: onDataChanged failed to update analyticsJson', e) }
        }
    }
            ...
            RowLayout {
                ...
                Repeater {
                    ...
                    Rectangle {
                        ...
                        // Phase 3.1: staggered list entrance (70ms per-item delay).
                        opacity: 0
                        transform: Translate { id: metricEntrance; y: 20 }
                        Component.onCompleted: metricEntranceTimer.start()
                        Timer {
                            id: metricEntranceTimer
                            interval: Math.min(metricCard.cardIndex * 70, 420)
                            repeat: false
                            onTriggered: metricEntranceAnim.start()
                        }
                        ParallelAnimation {
                            id: metricEntranceAnim
                            NumberAnimation { target: metricCard; property: "opacity"; to: 1; duration: 350; easing.type: Easing.OutCubic }
                            NumberAnimation { target: metricEntrance; property: "y"; to: 0; duration: 350; easing.type: Easing.OutCubic }
                        }

                        // Phase 3.2: hover/press micro-interaction.
                        Behavior on scale { NumberAnimation { duration: 120; easing.type: Easing.OutCubic } }
                        scale: metricHover.containsPress ? 0.98 : (metricHover.containsMouse ? 1.02 : 1.0)
                        MouseArea {
                            id: metricHover
                            anchors.fill: parent
                            hoverEnabled: true
                        ...
                        ColumnLayout {
                            ...
                            // Phase 3.1: spring-driven animated count.
                            AnimatedNumber {
                                value: modelData.rawValue
                                pixelSize: 20
                                color: theme.textPrimary
                                Layout.fillWidth: true
                ...
                Rectangle {
                    ...
                    Canvas {
                        ...
                        onPaint: {
                            ...
                            for (var i = 0; i < data.length; i++) {
                                ...
                                for (var m = 0; m < metrics.length; m++) {
                                    ...
                                    ctx.beginPath()
                                    // QML Canvas has no roundRect() — draw manually
                                    var r = 2
                                    ctx.moveTo(bx + r, by)
                                    ...
                                    // Comparison bars — show previous period data if available,
                                    // otherwise show a faded placeholder (no fabricated data)
                                    if (analyticsPage.compareMode) {
                                        var prevData = analyticsPage.previousPeriodData
                                        var lastWeekVal = 0
                                        if (prevData && prevData[platforms[i]] && prevData[platforms[i]][metrics[m]]) {
                                            lastWeekVal = prevData[platforms[i]][metrics[m]]
                                        }
                                        if (lastWeekVal > 0) {
                        ...
                        Connections {
                            ...
                            function onChartPlatformsChanged() {
            ...
            ColumnLayout {
                ...
                Repeater {
                    ...
                    Rectangle {
                        ...
                        // Phase 3.1: staggered entrance.
                        opacity: 0
                        transform: Translate { id: platformEntrance; y: 16 }
                        Component.onCompleted: platformEntranceTimer.start()
                        Timer {
                            id: platformEntranceTimer
                            interval: Math.min(index * 70, 420)
                            repeat: false
                            onTriggered: platformEntranceAnim.start()
                        }
                        ParallelAnimation {
                            id: platformEntranceAnim
                            NumberAnimation { target: platformCard; property: "opacity"; to: 1; duration: 300; easing.type: Easing.OutCubic }
                            NumberAnimation { target: platformEntrance; property: "y"; to: 0; duration: 300; easing.type: Easing.OutCubic }
                        }
                        // Phase 3.2: hover/press micro-interaction.
                        Behavior on scale { NumberAnimation { duration: 120; easing.type: Easing.OutCubic } }
                        scale: platformHover.containsPress ? 0.98 : (platformHover.containsMouse ? 1.02 : 1.0)
                        MouseArea {
                            id: platformHover
                            anchors.fill: parent
                            hoverEnabled: true
            ...
            ColumnLayout {
                ...
                Repeater {
                    ...
                    Rectangle {
                        ...
                        // Phase 3.1: staggered entrance.
                        opacity: 0
                        transform: Translate { id: topPostEntrance; y: 14 }
                        Component.onCompleted: topPostEntranceTimer.start()
                        Timer {
                            id: topPostEntranceTimer
                            interval: Math.min(index * 70, 420)
                            repeat: false
                            onTriggered: topPostEntranceAnim.start()
                        }
                        ParallelAnimation {
                            id: topPostEntranceAnim
                            NumberAnimation { target: topPostCard; property: "opacity"; to: 1; duration: 300; easing.type: Easing.OutCubic }
                            NumberAnimation { target: topPostEntrance; property: "y"; to: 0; duration: 300; easing.type: Easing.OutCubic }
                        }
                        // Phase 3.2: hover/press micro-interaction.
                        Behavior on scale { NumberAnimation { duration: 120; easing.type: Easing.OutCubic } }
                        scale: topPostHover.containsPress ? 0.98 : (topPostHover.containsMouse ? 1.02 : 1.0)
                        MouseArea {
                            id: topPostHover
                            anchors.fill: parent
                            hoverEnabled: true
