...
Page {
    ...
    // Smart per-post delete state
    property var pendingDeletePost: null
    property var pendingDeletePlatforms: []
    ...
    function batchCaptionChanged(platformKey, text) {
        if (!editingCaptions) editingCaptions = {}
        if (!editingCaptions[platformKey]) editingCaptions[platformKey] = []
        var idx = editingCaptions[platformKey].indexOf(text)
    ...
    // Posting helpers (#8 / #14)
    function safeVideoPath(post) {
        if (!post) return ""
        return post.video_path || post.local_path || post.path || ""
    }

    function preparePostPreview(post) {
        var videoPath = safeVideoPath(post)
        var caption = (post && post.caption) ? post.caption : ""
        ...
        if (typeof controller !== "undefined" && controller.previewPost) {
            try {
                ...
                console.warn("ContentPage: previewPost failed for single post", e)
        ...
        for (var i = 0; i < posts.length; i++) {
            ...
            if (typeof controller !== "undefined" && controller.previewPost) {
                try {
                    ...
                    console.warn("ContentPage: previewPost failed in batch for", videoPath, e)
    ...
    // ── Smart per-post delete (#per-platform delete) ──────────────
    ...
    // Find all platform entries for a post (by matching title = video_id).
    // The model creates one row per platform, so cross-posted videos appear
    // as multiple entries with the same title but different platform values.
    function findPostPlatformEntries(post) {
        var title = post.title || ""
        var entries = []
        for (var i = 0; i < filteredPosts.length; i++) {
            if (filteredPosts[i].title === title) {
                entries.push(filteredPosts[i])
            }
        }
        return entries
    }

    function openSmartDeleteDialog(post) {
        var entries = findPostPlatformEntries(post)
        var platforms = []
        for (var i = 0; i < entries.length; i++) {
            var p = entries[i].platform || ""
            if (p && platforms.indexOf(p) < 0)
                platforms.push(p)
        }
        pendingDeletePost = post
        pendingDeletePlatforms = platforms
        smartDeleteDialog.open()
    }

    function executeSmartDelete(platform) {
        if (!pendingDeletePost || typeof controller === "undefined")
            return
        var post = pendingDeletePost
        var entries = findPostPlatformEntries(post)

        if (platform === "") {
            // Delete from all platforms — iterate each platform entry
            for (var i = 0; i < entries.length; i++) {
                controller.deletePost(entries[i].postId, entries[i].platform || "")
            }
            showToast("Deleting from all platforms...", false)
        } else {
            // Find the correct postId for the target platform
            var targetPostId = post.postId
            for (var j = 0; j < entries.length; j++) {
                if (entries[j].platform === platform) {
                    targetPostId = entries[j].postId
                    break
                }
            }
            controller.deletePost(targetPostId, platform)
            showToast("Deleting from " + platformDisplayName(platform) + "...", false)
        }

        smartDeleteDialog.close()
        pendingDeletePost = null
        pendingDeletePlatforms = []
    }
                ...
                Repeater {
                    ...
                    Rectangle {
                        ...
                        ColumnLayout {
                            ...
                            Text {
                                text: "Caption " + (modelData.caption_length || 0) + (modelData.max_caption_length ? " / " + modelData.max_caption_length : "") + " chars"
                                font.pixelSize: 11
                                color: theme.textSecondary
                                Layout.fillWidth: true
                                elide: Text.ElideRight
    ...
    // ── Smart Per-Post Delete Dialog ──────────────────────────────
    // Shows when user clicks the trash button on a single post card.
    // Offers per-platform or all-platforms delete based on how many
    // platforms the post was published to.
    Dialog {
        id: smartDeleteDialog
        modal: true
        anchors.centerIn: parent
        ...
        contentItem: ColumnLayout {
            ...
            Text {
                text: contentPage.pendingDeletePost
                      ? "Delete '" + (contentPage.pendingDeletePost.title || "Untitled") + "'?"
                      : "Delete this post?"
                font.pixelSize: 14
                font.weight: Font.DemiBold
                color: theme.textPrimary
            ...
            // ── Per-platform delete buttons (shown when >1 platform) ──
            Repeater {
                model: contentPage.pendingDeletePlatforms.length > 1
                       ? contentPage.pendingDeletePlatforms : []

                Rectangle {
                    Layout.fillWidth: true
                    height: 38
                    radius: theme.radiusMd
                    color: perPlatformDeleteMouse.containsMouse ? Qt.rgba(0.85, 0.15, 0.15, 0.15) : theme.surfaceAlt
                    ...
                    RowLayout {
                        ...
                        Text {
                            text: "Delete from " + contentPage.platformDisplayName(modelData)
                            font.pixelSize: 13
                            color: theme.textPrimary
                            Layout.fillWidth: true
                        }
                    ...
                    MouseArea {
                        id: perPlatformDeleteMouse
                        anchors.fill: parent
                        hoverEnabled: true
                        cursorShape: Qt.PointingHandCursor
                        onClicked: contentPage.executeSmartDelete(modelData)
                        Accessible.name: "Delete from " + contentPage.platformDisplayName(modelData)
                        Accessible.role: Accessible.Button
                    }
                }
            }

            // ── Delete from all platforms (danger button) ──
            Rectangle {
                Layout.fillWidth: true
                height: 40
                radius: theme.radiusMd
                color: deleteAllMouseArea.containsMouse ? Qt.darker(theme.error, 1.15) : theme.error
                ...
                RowLayout {
                    ...
                    Text {
                        text: contentPage.pendingDeletePlatforms.length > 1
                ...
                MouseArea {
                    ...
                    hoverEnabled: true
                    cursorShape: Qt.PointingHandCursor
                    onClicked: contentPage.executeSmartDelete("")
                    Accessible.name: "Delete from all platforms"
                    Accessible.role: Accessible.Button
                }
            }
            ...
            RowLayout {
                ...
                Rectangle {
                    width: smartCancelLabel.implicitWidth + 32
                    height: 36
                    radius: theme.radiusMd
                    color: theme.surfaceAlt
                    Text {
                    ...
                    MouseArea {
                        ...
                        onClicked: {
                            smartDeleteDialog.close()
                            contentPage.pendingDeletePost = null
                            contentPage.pendingDeletePlatforms = []
                        }
                    }
                }
                        ...
                        Rectangle {
                            ...
                            Flickable {
                                ...
                                TextEdit {
                                    ...
                                    onTextChanged: batchCaptionChanged(platformKey, text)
    ...
    Flickable {
        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                Repeater {
                    ...
                    Rectangle {
                        ...
                        height: 32
                        radius: theme.radiusXl
                        color: contentPage.activeFilter === modelData ? theme.accent
                             : filterPillMouse.containsMouse ? theme.accentHover
                             : theme.surfaceCard
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    Rectangle {
                        ...
                        height: 28
                        radius: theme.radiusSm
                        color: clearMouse.containsPress ? theme.accentMuted
                             : clearMouse.containsMouse ? theme.accentHover
                             : theme.surfaceAlt
                        Behavior on color { ColorAnimation { duration: 120 } }
                        Text {
                            id: clearLabel
                    ...
                    Rectangle {
                        ...
                        height: 28
                        radius: theme.radiusSm
                        color: deleteSelectedMouse.containsMouse ? Qt.darker(theme.error, 1.12) : theme.error
                        Behavior on color { ColorAnimation { duration: 120 } }
                        Text {
                    ...
                    Rectangle {
                        ...
                        height: 28
                        radius: theme.radiusSm
                        color: postSelectedMouse.containsPress ? theme.accentMuted
                             : postSelectedMouse.containsMouse ? theme.accentHover
                             : theme.accent
                        Behavior on color { ColorAnimation { duration: 120 } }
                        Text {
            ...
            GridLayout {
                Layout.fillWidth: true
                // Phase 3.3: reduce columns below 900px (and step down under 1280).
                columns: contentPage.listViewMode ? 1
                         : (contentPage.width < 900 ? 1
                            : (contentPage.width < 1280 ? 2 : 3))
                ...
                Repeater {
                    ...
                    Rectangle {
                        ...
                        // Phase 3.1: staggered entrance (70ms per-item delay).
                        opacity: 0
                        transform: Translate { id: contentEntrance; y: 18 }
                        Component.onCompleted: contentEntranceTimer.start()
                        Timer {
                            id: contentEntranceTimer
                            interval: Math.min(index * 70, 560)
                            repeat: false
                            onTriggered: contentEntranceAnim.start()
                        }
                        ParallelAnimation {
                            id: contentEntranceAnim
                            NumberAnimation { target: contentCard; property: "opacity"; to: 1; duration: 320; easing.type: Easing.OutCubic }
                            NumberAnimation { target: contentEntrance; property: "y"; to: 0; duration: 320; easing.type: Easing.OutCubic }
                        }

                        // Phase 3.2: hover/press micro-interaction. Declared
                        // first so interactive children (checkbox, thumbnail)
                        // stay on top and keep receiving their own clicks.
                        Behavior on scale { NumberAnimation { duration: 120; easing.type: Easing.OutCubic } }
                        scale: contentHover.containsPress ? 0.98 : (contentHover.containsMouse ? 1.02 : 1.0)
                        MouseArea {
                            id: contentHover
                            anchors.fill: parent
                            hoverEnabled: true
                        ...
                        RowLayout {
                            ...
                            ColumnLayout {
                                ...
                                RowLayout {
                                    ...
                                    // Per-post delete button (smart delete)
                                    Rectangle {
                                        width: 24; height: 24
                                        radius: theme.radiusSm
                                        color: cardDeleteMouseArea.containsMouse ? Qt.rgba(1, 0, 0, 0.12) : "transparent"
                                        Text {
                                            anchors.centerIn: parent
                                            text: Icons.trash
                                            font.family: theme.iconFontFamily
                                            font.pixelSize: 12
                                            color: cardDeleteMouseArea.containsMouse ? theme.error : theme.textSecondary
                                        }
                                        MouseArea {
                                            id: cardDeleteMouseArea
                                            anchors.fill: parent
                                            hoverEnabled: true
                                            cursorShape: Qt.PointingHandCursor
                                    ...
                                    Text {
                                        text: {
                                            ...
                                            } catch(e) { console.warn('ContentPage: relative timestamp parse failed', ts, e); return ts }
                                        }
                                        font.pixelSize: 11
                                        color: theme.textMuted
                                    }
            ...
            RowLayout {
                ...
                Rectangle {
                    ...
                    height: 32
                    radius: theme.radiusMd
                    color: contentPage.currentPage > 1
                           ? (prevMouse.containsMouse ? theme.accentHover : theme.surfaceCard)
                           : theme.surfaceAlt
                    Behavior on color { ColorAnimation { duration: 120 } }
                ...
                Rectangle {
                    ...
                    height: 32
                    radius: theme.radiusMd
                    color: contentPage.currentPage < contentPage.totalPages
                           ? (nextMouse.containsMouse ? theme.accentHover : theme.surfaceCard)
                           : theme.surfaceAlt
                    Behavior on color { ColorAnimation { duration: 120 } }
