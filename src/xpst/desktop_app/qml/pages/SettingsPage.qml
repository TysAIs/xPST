...
Page {
    ...
    // Maps a shortcut label to the lowercase action key used by the backend.
    function shortcutActionKey(label) {
        if (label === "Refresh") return "refresh"
        if (label === "Quit") return "quit"
        return (label || "").toLowerCase()
    }

    function loadShortcuts() {
        if (typeof controller === "undefined" || !controller.getShortcuts) return
        try {
            var saved = JSON.parse(controller.getShortcuts())
            var defs = shortcutDefs.slice()
            for (var i = 0; i < defs.length; i++) {
                var key = shortcutActionKey(defs[i].label)
                if (saved[key]) {
                    defs[i] = {
                        label: defs[i].label,
                        key: saved[key],
                        defaultKey: defs[i].defaultKey
                    }
                }
            }
            shortcutDefs = defs
    ...
    function toggleMcpServer() {
        ...
        if (mcpRunning) {
            try {
                var res = JSON.parse(controller.stopMcpServer())
                mcpRunning = res.ok ? false : mcpRunning
                showToast(res.ok ? "MCP server stopped" : ("Failed to stop MCP: " + (res.error || "")), !res.ok)
            } catch(e) {
                console.error("Failed to stop MCP server", e)
                showToast("Failed to stop MCP server", true)
            }
        } else {
            try {
                var res2 = JSON.parse(controller.startMcpServer())
                mcpRunning = res2.ok ? true : mcpRunning
                showToast(res2.ok ? "MCP server started" : ("Failed to start MCP: " + (res2.error || "")), !res2.ok)
            } catch(e) {
                console.error("Failed to start MCP server", e)
                showToast("Failed to start MCP server", true)
            }
        }
    }
                ...
                Rectangle {
                    ...
                    ColumnLayout {
                        ...
                        RowLayout {
                            ...
                            Switch {
                                checked: settingsPage.notifCompletion
                                onCheckedChanged: {
                                    settingsPage.notifCompletion = checked
                        ...
                        RowLayout {
                            ...
                            Switch {
                                checked: settingsPage.notifErrors
                                onCheckedChanged: {
                                    settingsPage.notifErrors = checked
                                    settingsPage.saveNotifications()
                                }
    ...
    // Developer / MCP Server Section
            ...
            RowLayout {
                ...
                Rectangle {
                    ...
                    height: 40
                    radius: theme.radiusMd
                    color: hasErrors ? theme.surfaceAlt
                         : saveMouse.containsPress ? theme.accentMuted
                         : saveMouse.containsMouse ? theme.accentHover
                         : theme.accent
                ...
                Rectangle {
                    ...
                    height: 40
                    radius: theme.radiusMd
                    color: resetSettingsMouse.containsPress ? theme.accentMuted
                         : resetSettingsMouse.containsMouse ? theme.accentHover
                         : theme.surfaceAlt
                    Behavior on color { ColorAnimation { duration: 120 } }
