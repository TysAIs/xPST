...
Page {
    ...
    property var platformHealthData: {
        ...
        } catch(e) { console.error('DashboardPage: failed to parse platformHealth', e); if (typeof showToast !== "undefined") showToast("Could not load platform status", true) }
        return ({})
    }
    property var recentPostsData: {
        ...
        } catch(e) { console.error('DashboardPage: failed to parse recentPosts', e); if (typeof showToast !== "undefined") showToast("Could not load recent posts", true) }
        return ([])
    }
    ...
    function loadReadiness() {
        ...
        } catch(e) { console.error('DashboardPage: loadReadiness failed', e); if (typeof showToast !== "undefined") showToast("Could not check setup readiness", true) }
    }
    ...
    function repairOrOpenConnections() {
        ...
        if (dashboardPage.canRepairLocalSetup() && controller.repairReadiness) {
            ...
            } catch(e) { console.error('DashboardPage: repairReadiness failed', e); if (typeof showToast !== "undefined") showToast("Could not repair local setup", true) }
        }
    ...
    Flickable {
        ...
        // Phase 3.1: loading skeleton (animated shimmer) replacing the
        // BusyIndicator overlay — shown until the first data refresh lands.
        ColumnLayout {
            anchors.centerIn: parent
            spacing: theme.spacingMd
            visible: !dashboardPage.dataLoaded
            z: 10
            width: 280
            ...
            Repeater {
                ...
                LoadingSkeleton {
                    ...
                    running: !dashboardPage.dataLoaded
                }
            }
        }
        ...
        ColumnLayout {
            ...
            // Fade content in once data has loaded so the spinner reads cleanly.
            opacity: dashboardPage.dataLoaded ? 1.0 : 0.3
            ...
            RowLayout {
                ...
                Repeater {
                    model: [
                        { label: "Total Posts",     value: typeof controller !== "undefined" ? controller.totalPosts : "0",   icon: Icons.stats },
                        { label: "Total Reach",     value: typeof controller !== "undefined" ? controller.totalReach : "0",   icon: Icons.users },
                        { label: "Best Platform",   value: typeof controller !== "undefined" ? controller.bestPlatform : "—", icon: Icons.trophy },
                        { label: "Posts This Week",  value: typeof controller !== "undefined" ? controller.postsThisWeek : "0", icon: Icons.calendar }
                    ]
                    ...
                    Rectangle {
                        ...
                        radius: theme.radiusLg
                        color: cardMouse.containsMouse ? theme.surfaceAlt : theme.surfaceCard
                ...
                RowLayout {
                    ...
                    Repeater {
                        ...
                        Rectangle {
                            ...
                            radius: theme.radiusLg
                            color: healthCardMouse.containsMouse ? theme.surfaceAlt : theme.surfaceCard
            ...
            ColumnLayout {
                ...
                GridLayout {
                    ...
                    Repeater {
                        ...
                        Rectangle {
                            ...
                            radius: theme.radiusLg
                            color: recentCardMouse.containsMouse ? theme.surfaceAlt : theme.surfaceCard
                            Behavior on color { ColorAnimation { duration: 120 } }
                ...
                Rectangle {
                    ...
                    Layout.preferredHeight: 80
                    color: "transparent"
                    visible: dashboardPage.dataLoaded && (!dashboardPage.recentPostsData || dashboardPage.recentPostsData.length === 0)
