...
# Real package version for the About page (falls back if package metadata
# is unavailable, e.g. running from a source checkout without install).
try:
...
class AppController(QObject):
    ...
    # appVersion(str) — real version from the xpst package (About page).
    ...
    def _refresh_recent_posts(self) -> None:
        ...
        try:
            ...
            # Try to get analytics store for per-post metrics
            ...
            for video_id, vdata in posted.items():
                ...
                for platform, pinfo in vdata.get("posted_to", {}).items():
                    post_id = pinfo.get("id") or video_id
                    entry: dict[str, Any] = {
                        "title": video_id,
                        "caption": caption,
                        "platform": platform,
                    ...
                    # Enrich with analytics metrics if available
                    if analytics_store:
                        try:
                            snapshots = analytics_store.latest_for_post(platform, post_id)
                            if snapshots:
                                latest_snap = snapshots[-1]
                                entry["views"] = latest_snap.get("views", 0)
                                entry["likes"] = latest_snap.get("likes", 0)
                                entry["comments"] = latest_snap.get("comments", 0)
                                entry["shares"] = latest_snap.get("shares", 0)
                        except Exception:
                            pass

                    # Ensure platforms dict exists for DetailPanel compatibility
                    if "platforms" not in entry:
                        entry["platforms"] = {}

                    posts.append(entry)

            # Build per-platform data for DetailPanel
            # Group by video_id so DetailPanel can show all platforms for a video
            ...
            for post in posts:
                ...
                video_groups[vid]["platforms"][platform] = {
            ...
            # Output as flat list (for ContentPage) with platforms dict (for DetailPanel)
            output = []
            for _vid, group in video_groups.items():
                for platform, metrics in group["platforms"].items():
                    entry = {
                        "title": group["title"],
    ...
    @Slot(str, str, str)
    def postVideo(self, video_path: str, caption: str, platforms_json: str = "") -> None:
        """Post a video file to selected (or all) enabled platforms.
        ...
        Args:
            ...
            platforms_json: JSON array of platform names to post to.
                Empty string means all enabled platforms.
        """
        preview = json.loads(self.previewPost(video_path, caption, platforms_json))
        ...
        # Parse selected platforms (empty = all enabled)
        selected_platforms: list[str] | None = None
        if platforms_json:
            try:
                selected_platforms = json.loads(platforms_json)
        ...
        def _run() -> None:
            try:
                ...
                result = loop.run_until_complete(
                    self._engine.post_manual(path, caption, platforms=selected_platforms)
                )
                loop.close()
    ...
    def startMcpServer(self) -> str:
        """Start the MCP server (``xpst mcp``) as a background subprocess.

        Returns JSON ``{"ok": true}`` on success or ``{"ok": false, "error": ...}``.
        """
        if self._mcp_process is not None and self._mcp_process.poll() is None:
            return json.dumps({"ok": True, "running": True, "message": "already running"})
        try:
            import sys

            cmd = [sys.executable, "-m", "xpst", "mcp"]
            ...
            logger.info("MCP server started (pid=%s)", self._mcp_process.pid)
            return json.dumps({"ok": True, "running": True})
        except Exception as exc:
            logger.error("startMcpServer failed: %s", exc)
            self._mcp_process = None
            return json.dumps({"ok": False, "error": str(exc)})

    @Slot(result=str)
    def stopMcpServer(self) -> str:
        """Stop the running MCP server subprocess if one is active."""
        if self._mcp_process is None or self._mcp_process.poll() is not None:
            self._mcp_process = None
            return json.dumps({"ok": True, "running": False, "message": "not running"})
        try:
            ...
            logger.error("stopMcpServer failed: %s", exc)
            return json.dumps({"ok": False, "error": str(exc)})

    @Slot(result=bool)
    def isMcpRunning(self) -> bool:
        """Return True if the MCP subprocess is currently running."""
    ...
    @Slot(result=str)
    def hasCompletedSetup(self) -> str:
        """Return whether first-run setup has been completed.

        Checks for the presence of a config file under the xPST config
        directory (``~/.xpst`` on POSIX, ``%APPDATA%/xpst`` on Windows) and
        reports the persisted ``first_run_complete`` flag. QML uses this to
        decide whether to show the first-run OnboardingPage wizard.

        Returns:
            JSON string ``{"ok": true, "exists": bool, "first_run_complete": bool}``.
        """
        try:
            config_dir = Path(getattr(self._config, "config_dir", "")) or get_config_dir()
            config_dir = config_dir.expanduser()
            config_file = config_dir / "config.yaml"
            exists = config_file.exists()
            first_run = bool(getattr(self._config, "first_run_complete", False))
            return json.dumps(
                {"ok": True, "exists": bool(exists), "first_run_complete": first_run},
                default=str,
            )
        except Exception as exc:
            logger.error("hasCompletedSetup error: %s", exc)
            return json.dumps({"ok": False, "exists": False, "first_run_complete": False, "error": str(exc)})

    @Slot(result=str)
    def markOnboardingComplete(self) -> str:
        """Persist ``first_run_complete = True`` so the wizard is not shown again.

        Called by OnboardingPage when the user finishes the guided setup.
        Reloads the config object after saving so the flag is consistent for
        ...
        try:
            ...
            config_dir = Path(getattr(self._config, "config_dir", "")).expanduser()
            config_path = config_dir / "config.yaml"
            # Ensure the directory + a config file exist before saving so a
            # brand-new install (no config.yaml yet) persists cleanly.
            config_dir.mkdir(parents=True, exist_ok=True)
            self._config.save(str(config_path))
            # Reload so downstream code sees the persisted state.
            if XPSTConfig is not None:
                self._config = XPSTConfig.load(str(config_path))
            self._engine = None
            self.refreshData()
            logger.info("First-run onboarding marked complete.")
            return json.dumps({"ok": True})
        except Exception as exc:
            logger.error("markOnboardingComplete error: %s", exc)
            return json.dumps({"ok": False, "error": str(exc)})
    ...
    @Slot(result=str)
    def installFFmpeg(self) -> str:
        """One-click FFmpeg install (E6.1). Detects platform and runs
        ...
        try:
            if system == "Darwin":
                ...
                apt = shutil.which("apt-get") or shutil.which("dnf")
                if apt:
                ...
                return json.dumps({"ok": False, "error": "No supported package manager found."})
            elif system == "Windows":
                return json.dumps({
                    "ok": False,
                    "error": "Download FFmpeg from https://ffmpeg.org/download.html and add to PATH.",
                    "url": "https://ffmpeg.org/download.html",
                })
            return json.dumps({"ok": False, "error": f"Unsupported platform: {system}"})
        except Exception as exc:
            logger.error("installFFmpeg error: %s", exc)
            return json.dumps({"ok": False, "error": str(exc)})
    ...
    def getLocalVideos(self, folder_path: str = "") -> str:
        ...
        if not folder_path:
            folder_path = getattr(self._config, "local_path", "") if self._config else ""
        if not folder_path:
            folder_path = str(Path.home() / "Movies")

        scan_path = Path(folder_path).expanduser()
        if not scan_path.is_dir():
            return json.dumps({"ok": False, "error": f"Directory not found: {scan_path}", "videos": []})

        all_exts = VIDEO_EXTENSIONS | IMAGE_EXTENSIONS
        files = []
        for ext in all_exts:
            files.extend(scan_path.glob(f"*{ext}"))
            files.extend(scan_path.glob(f"*{ext.upper()}"))
    ...
    def browseForFolder(self) -> str:
        ...
        # QFileDialog needs to run on the main thread — this slot is called from QML
        # which runs on the main thread, so it's fine.
        folder = QFileDialog.getExistingDirectory(
            None,
            QCoreApplication.translate("AppController", "Select Video Folder"),
            str(Path.home()),
        )
        return folder or ""

    # ── Scheduled posts (SchedulePage bridge to ScheduleManager) ──
    # ScheduleManager (xpst.schedule_manager) persists entries in
    # ~/.xpst/schedule.json. These slots expose list/add/remove to QML so the
    # Schedule page can manage real scheduled posts instead of showing a
    # "coming soon" placeholder.
    @Slot(result=str)
    def listScheduledPosts(self) -> str:
        """Return all scheduled posts as JSON: {"ok": true, "posts": [...]}."""
        try:
            from xpst.schedule_manager import ScheduleManager
            manager = ScheduleManager()
            entries = manager.list()
            return json.dumps({"ok": True, "posts": entries}, default=str)
        except Exception as exc:
            logger.error("listScheduledPosts error: %s", exc)
            return json.dumps({"ok": False, "posts": [], "error": str(exc)})

    @Slot(str, result=str)
    def addScheduledPost(self, post_json: str) -> str:
        """Add a scheduled post.

        Args:
            post_json: JSON string with keys: video_path, caption,
                scheduled_time (ISO or 'YYYY-MM-DD HH:MM'), platforms (list),
                repeat_rule ('one_time'|'daily'|'weekly').

        Returns:
            JSON string {"ok": true, "post": <entry>} or {"ok": false, "error": ...}.
        """
        try:
        ...
        scheduled_time_raw = str(data.get("scheduled_time", "")).strip()
        platforms = data.get("platforms") or []
        repeat = str(data.get("repeat_rule", "one_time")).strip().lower()

        if not video_path:
            return json.dumps({"ok": False, "error": "No video file selected"})
        if not scheduled_time_raw:
            return json.dumps({"ok": False, "error": "No scheduled time provided"})

        # Parse the scheduled time (support ISO and 'YYYY-MM-DD HH:MM').
        dt = None
        for fmt in ("%Y-%m-%dT%H:%M:%S", "%Y-%m-%d %H:%M:%S", "%Y-%m-%d %H:%M", "%Y-%m-%dT%H:%M"):
            try:
                dt = datetime.strptime(scheduled_time_raw, fmt)
                break
            except ValueError:
                continue
        if dt is None:
            try:
                dt = datetime.fromisoformat(scheduled_time_raw)
            except ValueError:
                return json.dumps({"ok": False, "error": f"Invalid date format: {scheduled_time_raw}"})

        # Map UI recurrence values to ScheduleManager repeat rules.
        repeat_map = {"one_time": None, "one-time": None, "once": None,
                      "daily": "daily", "weekly": "weekly", "monthly": "monthly"}
        repeat_rule = repeat_map.get(repeat)
        ...
        try:
            ...
            self.dataChanged.emit()
            return json.dumps({"ok": True, "post": entry}, default=str)
        except Exception as exc:
            logger.error("addScheduledPost error: %s", exc)
            return json.dumps({"ok": False, "error": str(exc)})

    @Slot(str, result=str)
    def removeScheduledPost(self, entry_id: str) -> str:
        """Remove a scheduled post by ID. Returns {"ok": bool, ...}."""
        try:
            from xpst.schedule_manager import ScheduleManager
            manager = ScheduleManager()
            removed = manager.remove(entry_id)
            self.dataChanged.emit()
            return json.dumps({"ok": removed, "id": entry_id}, default=str)
        except Exception as exc:
...
class ThemeProvider(QObject):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self._dark_mode = self._load_dark_mode_pref()

    def _load_dark_mode_pref(self) -> bool:
        """Load dark mode preference from user settings on startup."""
        try:
            import json as _json
            from pathlib import Path as _Path

            from xpst.utils.platform import get_config_dir
            settings_path = _Path(get_config_dir()) / "settings.json"
    ...
    # Analytics metric icons (G22: replace text-as-icons with real glyphs).
    @Property(str, constant=True)
    def iconEye(self): return icon_glyphs.glyph("eye")
    @Property(str, constant=True)
    def iconHeart(self): return icon_glyphs.glyph("heart")
    @Property(str, constant=True)
    def iconMessage(self): return icon_glyphs.glyph("message")
    @Property(str, constant=True)
    def iconShare(self): return icon_glyphs.glyph("share")
    @Property(str, constant=True)
    def iconTrending(self): return icon_glyphs.glyph("trending")

    # About / link icons
    @Property(str, constant=True)
    def iconHome(self): return icon_glyphs.glyph("home")
    @Property(str, constant=True)
    def iconCode(self): return icon_glyphs.glyph("code")
    @Property(str, constant=True)
    def iconBook(self): return icon_glyphs.glyph("book")
    @Property(str, constant=True)
    def iconBug(self): return icon_glyphs.glyph("bug")
    @Property(str, constant=True)
    def iconFileText(self): return icon_glyphs.glyph("filetext")
    @Property(str, constant=True)
    def iconExternal(self): return icon_glyphs.glyph("external")

    # Dark mode toggle — persisted to user settings
    ...
    def darkMode(self, value):
        if self._dark_mode != value:
            ...
            try:
                ...
                pass  # Non-fatal — dark mode still works in-session
