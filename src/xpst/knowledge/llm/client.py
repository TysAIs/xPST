...
class LLMClient:
    ...
    def is_reachable(self, *, timeout: float = 5.0) -> bool:
        """Lightweight reachability probe: GET ``{base_url}/models`` with a
        short timeout. Returns True on any 2xx response, False on connection
        error / non-2xx. Used by the ingest pipeline to emit a clear notice and
        fall back to deterministic extraction when the configured LLM is down
        (Phase 4.5 / 4.4), so an unreachable LLM never silently stalls
        ingestion."""
        try:
            with _httpx_client(timeout) as http:
                resp = http.get(f"{self._base_url}/models",
