...
class Nugget:
    ...
    # Phase 4: reframed extraction. A nugget is now one of several extraction
    # "modes" (teachable point, repurposable clip, topic summary). ``title``,
    # ``topic`` and ``tags`` carry the richer metadata a clip/topic mode emits;
    # the classic nugget mode leaves them None/empty so existing data and tests
    # are unaffected. The id stays a function of (source_video_id, point) so
    # adding fields never re-keys existing nuggets.
    title: str | None = None
    topic: str | None = None
    ...
    def from_dict(cls, d: dict) -> Nugget:
        ...
        # Phase 4 fields are optional; older stores do not carry them.
        d.setdefault("source_platform", None)
        d.setdefault("source_post_id", None)
...
class QueueItem:
    ...
    seq: int = 0
    # Phase 4 auto-ingest provenance: the platform + post id the enqueued
    # source was published through, so drained nuggets carry performance
    # history. Optional — a manual ``kb add`` enqueues with None.
    source_platform: str | None = None
    source_post_id: str | None = None

    @classmethod
    def create(cls, *, source: str, workspace: str = "default",
               enqueued_at: float = 0.0, seq: int = 0,
               source_platform: str | None = None,
               source_post_id: str | None = None) -> QueueItem:
        return cls(
    ...
    def from_dict(cls, d: dict) -> QueueItem:
        ...
        d.setdefault("seq", 0)
        # Phase 4 provenance fields are optional on older queue files.
        d.setdefault("source_platform", None)
        d.setdefault("source_post_id", None)
