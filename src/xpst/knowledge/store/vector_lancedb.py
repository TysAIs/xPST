...
class LanceDBStore(KnowledgeStore):
    ...
    def _open(self, name: str, sample_row: dict | None):
        db = self._conn()
        if name in (db.list_tables() if hasattr(db, "list_tables") else db.table_names()):
    ...
    def add_nugget(self, nugget: Nugget) -> None:
        ...
        db = self._conn()
        if _NUGGETS in (db.list_tables() if hasattr(db, "list_tables") else db.table_names()):
    ...
    def upsert_area(self, area: Area) -> None:
        ...
        if _AREAS in (db.list_tables() if hasattr(db, "list_tables") else db.table_names()):
    ...
    def areas(self) -> list[Area]:
        db = self._conn()
        if _AREAS not in (db.list_tables() if hasattr(db, "list_tables") else db.table_names()):
            return []
