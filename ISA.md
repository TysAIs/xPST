...
- [ ] ISC-1: kb doctor `diagnose()` no longer mutates state; regression test passes
- [ ] ISC-2: AnalyticsPage.qml:205 (and all page combos) render glyphs via split layout with font.family applied; no literal glyph codepoints in label strings (grep probe)
- [ ] ISC-3: Full-repo bug sweep completed with every confirmed finding either fixed or ticketed in AUDIT doc with severity
- [ ] ISC-4: Full pytest suite passes 0 failed / 0 error on Linux
- [ ] ISC-5: ruff + mypy + import-linter + pip-audit all clean at HEAD
...
- [ ] ISC-8: Inventory doc lists every integrated/vendored open-source project with version, role, license, and adapter seam path
- [ ] ISC-9: `xpst doctor` (or equivalent) reports available updates for each integrated project
...
- [ ] ISC-12: Anti: no integration update path auto-applies an upstream update without explicit user action
...
- [ ] ISC-14: Partial failure handling: one platform failing does not abort the others; retry path exists and is tested
- [ ] ISC-15: Double-post prevention holds across retries and restarts (existing tests stay green, new fan-out paths covered)
...
### D. Video quality
- [ ] ISC-17: Upload pipeline performs no silent re-encode when the source already satisfies platform constraints (probe: hash/bitrate comparison test)
- [ ] ISC-18: When transcode is required, settings are quality-first (CRF/bitrate floor documented and tested), not library defaults
- [ ] ISC-19: Per-platform encode profiles exist and are covered by tests comparing output resolution/bitrate against source
- [ ] ISC-20: Quality report surfaced to user after each upload (what was sent: resolution, bitrate, transcode yes/no)
- [ ] ISC-21: Anti: no upload path downscales or recompresses media beyond the platform's published minimum requirements
...
- [ ] ISC-23: Metrics normalized into one schema with platform-specific fields preserved
...
- [ ] ISC-26: Unavailable metrics per platform documented (capability matrix), shown as N/A not zero
...
- [ ] ISC-30: docs/MCP_TOOLS.md and AGENT_GUIDE.md regenerated and accurate against the live tool registry (drift probe)
- [ ] ISC-31: Anti: no MCP tool or CLI command can print credentials/tokens to output
...
- [ ] ISC-36: KB queryable via MCP (kb_query) and CLI with cited sources in results
- [ ] ISC-37: KB doctor diagnoses all six health checks without mutating state (ties ISC-1)
- [ ] ISC-38: Ingestion queue durable across restarts (existing Phase 5 work verified, not just merged)
- [ ] ISC-39: Anti: KB never embeds or stores content from platforms the user has not connected or links the user has not explicitly saved
...
- [ ] ISC-46: README rewritten: what it is, what it does, screenshots, quickstart, agent-integration section, architecture pointer, comparison framing — reviewed against top-tier OSS READMEs
- [ ] ISC-47: CONTRIBUTING, SECURITY, LICENSE, CHANGELOG current and consistent with the release
...
- [ ] ISC-51: Anti: no secrets, tokens, or owner-identifying paths in any committed file (scan probe)
...
- [ ] ISC-53: Lazy-load walls hold: importing CLI/core does not pull heavy deps (import-linter contracts + runtime probe)
...
- [ ] ISC-61: Antecedent: research spec (docs/XPST-NORTH-STAR.md) exists and was reviewed by owner before the build session starts
- [ ] ISC-62: Antecedent: owner-approved goal prompt kicked off the build session (this gates everything downstream)
- [ ] ISC-63: Carry-over register empty: every known defect from prior sessions fixed or explicitly accepted by owner
- [ ] ISC-64: Anti: no upstream project vendored wholesale when a pinned dependency + adapter seam suffices (bloat guard)

### M. North-star derived criteria (spec §7, gap-register mapped)


#### NS — Architecture / state
- [ ] ISC-65: `grep -n 'self._sources.get("tiktok")' src/xpst/engine.py` returns nothing in `_process_video`.
- [ ] ISC-66: `rg "self._sources.get\(source\)" src/xpst/engine.py` matches in `_process_video`.
- [ ] ISC-67: `pytest -k process_video_nontiktok` passes (posting from a fake non-tiktok source downloads via that source).
- [ ] ISC-68: `grep -n 'del self._state\["posted_videos"\]' src/xpst/state.py` returns nothing.
- [ ] ISC-69: `pytest -k clear_dlq_preserves_record` passes.
- [ ] ISC-70: `rg 'f"tiktok:\{video_id\}"' src/xpst/state.py` returns nothing.
- [ ] ISC-71: `rg 'source_platform=""' src/xpst/state.py` returns nothing.
...
- [ ] ISC-74: `test -d src/xpst/usecases && echo present || echo gone` prints "gone" (or `rg "from xpst.usecases" src/ --glob '!usecases/**'` returns nothing).
- [ ] ISC-75: `pytest tests/test_engine_consolidation.py` passes (single engine, no engine_v2).
...
#### NS — Posting parity
- [ ] ISC-80: `pytest -k cross_flow_dedup` passes (same video via both flows = one record).
- [ ] ISC-81: `rg "compute_caption_hash" src/xpst/monitor.py` is gone OR superseded by file-hash.
- [ ] ISC-82: `rg "content_hash" src/xpst/utils/content_hash.py` exists and is imported by the upload path.
- [ ] ISC-83: `pytest -k retry_no_double_post` passes (post-success network blip does not re-upload).
...
- [ ] ISC-86: `pytest -k manual_post_idempotent` passes.
- [ ] ISC-87: `rg "video_path.stem" src/xpst/engine.py` not used as a state key.
- [ ] ISC-88: `pytest -k deferred_not_failed` passes (anti-bot deferral != DLQ failure).
...
- [ ] ISC-94: `rg "source_platform" src/xpst/services/upload_service.py` shows it is passed through.

#### NS — Video quality
- [ ] ISC-95: `rg "scale=-2:\{resolution\}" src/xpst/utils/video.py` returns nothing.
- [ ] ISC-96: `rg "if\(gt\(a,1\)|long.?edge|1920" src/xpst/utils/video.py` shows orientation-aware scaling.
- [ ] ISC-97: `python -c "from xpst.config import VideoConfig as V; assert V().encoding_instagram.resolution>=1080"`.
- [ ] ISC-98: `python -c "from xpst.config import VideoConfig as V; assert V().encoding_instagram.profile=='high'"`.
- [ ] ISC-99: `pytest -k vertical_1080x1920_preserved` passes (output width >= 1080 for 9:16 source).
- [ ] ISC-100: `rg '"-r", *"30"|fps=30' src/xpst/utils/video.py` returns nothing (conditional fps only).
- [ ] ISC-101: `pytest -k fps_preserved_60` passes.
- [ ] ISC-102: `rg "get_video_info" src/xpst/services/upload_service.py` shows a compliance/passthrough probe.
- [ ] ISC-103: `pytest -k passthrough_skips_compliant` passes.
- [ ] ISC-104: `rg "media_category" src/xpst/platforms/x.py` shows `'tweet_video'`.
- [ ] ISC-105: `rg "bv\*\+ba|merge-output-format" src/xpst/sources/youtube.py` matches.
- [ ] ISC-106: `rg "bv\*\+ba|merge-output-format" src/xpst/sources/tiktok.py` matches.
- [ ] ISC-107: `pytest -k bufsize_unit_parse` passes for "3.5M".
...
#### NS — Analytics
- [ ] ISC-110: `python -c "import instagrapi,inspect; c=instagrapi.Client; assert not hasattr(c,'load_session')"` (confirms old API is gone — code must not call it).
- [ ] ISC-111: `rg "load_session|insights\.get_media_insights" src/xpst/analytics.py` returns nothing.
- [ ] ISC-112: `rg "load_settings|insights_media" src/xpst/analytics.py` matches.
- [ ] ISC-113: `pytest -k analytics_instagram_real_api` passes WITHOUT a MagicMock fabricating the method.
- [ ] ISC-114: `rg "get_media_insights" tests/test_analytics.py` returns nothing.
- [ ] ISC-115: `pytest -k analytics_payload_has_metrics` passes (backend `summary.total_views` populated).
- [ ] ISC-116: `rg "platforms\[i\]\.total_views" src/xpst/desktop_app/qml/pages/AnalyticsPage.qml` resolves to a key the backend actually sets.
- [ ] ISC-117: `rg "\[0.72|0.65|0.8|0.58\]" src/xpst/desktop_app/qml/pages/AnalyticsPage.qml` returns nothing.
- [ ] ISC-118: `python -c "from xpst.analytics import AnalyticsCollector"` and a persistence table/file exists after one collection run.
- [ ] ISC-119: `pytest -k analytics_persists_snapshot` passes.
- [ ] ISC-120: `rg "asyncio.run" src/xpst/dashboard/analytics.py` not called on the Qt/GUI thread (collection in a worker).
...
- [ ] ISC-126: `xpst analytics --json | python -c "import sys,json; json.load(sys.stdin)"` exits 0.
- [ ] ISC-127: `rg "if as_json" src/xpst/cli.py` matches in the analytics command body.
- [ ] ISC-128: `rg '"enum": \["youtube", "x", "instagram"\]' src/xpst/mcp/server.py` returns nothing (enums dynamic).
- [ ] ISC-129: `pytest -k mcp_enum_from_providers` passes.
- [ ] ISC-130: `python -c "import json; ..."` — `xpst_config_show` MCP output contains no `dashboard_password_hash`.
- [ ] ISC-131: `rg "_mask_sensitive_values" src/xpst/mcp/server.py` matches (CLI masker reused).
- [ ] ISC-132: `rg "config.monitoring.__dict__" src/xpst/mcp/server.py` returns nothing (or is masked).
- [ ] ISC-133: `pytest -k xpst_run_returns_per_post` passes (results include post URLs, not just a string).
- [ ] ISC-134: `rg "xpst_analytics" src/xpst/mcp/server.py` matches (MCP analytics tool exists).
- [ ] ISC-135: `rg "xpst_schedule_add" src/xpst/mcp/server.py` matches OR scheduling documented CLI-only.
- [ ] ISC-136: `pytest tests/test_mcp_server.py` passes.
...
#### NS — Knowledge base
- [ ] ISC-140: `rg "_requeue_stale\(persist=True\)" src/xpst/knowledge/queue.py` is guarded by a read-only flag for doctor.
- [ ] ISC-141: `pytest -k doctor_readonly` passes (doctor does not rewrite queue.json).
- [ ] ISC-142: `pytest -k workspace_resolve_no_mkdir_on_read` passes.
- [ ] ISC-143: `rg "needle in n.point.lower\(\)" src/xpst/knowledge/mcp/tools.py` returns nothing.
- [ ] ISC-144: `rg "store.search\(" src/xpst/knowledge/mcp/tools.py` matches (semantic query exposed).
- [ ] ISC-145: `pytest -k kb_query_semantic` passes (embedding-based, not substring).
- [ ] ISC-146: `rg "k=|limit=|score" src/xpst/knowledge/mcp/tools.py` shows result limit + score on kb_query.
- [ ] ISC-147: `pytest -k kb_query_provenance` passes (result carries source URL + timestamp).
- [ ] ISC-148: `rg "LanceDBStore" src/xpst/knowledge/cli_kb.py` matches (LanceDB default-when-installed) OR documented JSON-only.
- [ ] ISC-149: `pytest -k kb_content_hash_dedup` passes (same video via 2 URLs ingests once).
- [ ] ISC-150: `rg "write_text" src/xpst/knowledge/store/json_store.py` replaced by atomic tempfile+replace.
- [ ] ISC-151: `pytest -k kb_store_corruption_tolerant` passes.
- [ ] ISC-152: `python -c "from xpst.knowledge.llm.embeddings import EndpointEmbedder as E; assert hasattr(E,'model_name') or 'model_name' in E.__init__.__code__.co_names"`.
- [ ] ISC-153: `rg "def reembed|kb reembed" src/xpst/knowledge` matches OR roadmap-documented.
...
- [ ] ISC-168: `rg "sys.frozen" src/xpst/updater.py` matches (frozen guard).
- [ ] ISC-169: `pytest -k updater_frozen_guard` passes (no pip call when frozen).
- [ ] ISC-170: `rg "pip install --upgrade" src/xpst/updater.py` is constrained (pins) and followed by a smoke/rollback.
- [ ] ISC-171: `rg "authlib" NOTICES.md LICENSING_REPORT.md` returns nothing (or authlib is actually a dep).
- [ ] ISC-172: `python -c "import tomllib;d=tomllib.load(open('pyproject.toml','rb'));print([x for x in d['project']['optional-dependencies']['knowledge'] if '<' not in x])"` prints `[]` (upper bounds present).
- [ ] ISC-173: `gh workflow list` shows one CI workflow (test.yml deleted).
- [ ] ISC-174: `rg "feat/knowledge-base" .github/workflows/ci.yml` matches (branch trigger) — or default branch updated.
...
#### NS — Docs front door
- [ ] ISC-180: `rg -i "knowledge|kb " README.md` matches (KB documented at front door).
- [ ] ISC-181: `rg -i "tiktok" README.md` does not present TikTok as a posting destination.
- [ ] ISC-182: `grep -c "command" README.md` reflects the actual count (25) consistently with CHANGELOG.
- [ ] ISC-183: `rg "9 tools|9 MCP" README.md docs/MCP_TOOLS.md` returns nothing (13 documented).
- [ ] ISC-184: `rg "866 passing" README.md docs/ENTERPRISE_READINESS.md` returns nothing (or matches audited number).
- [ ] ISC-185: `rg "pip install xpst" README.md` is gated by a published-on-PyPI note or removed.
- [ ] ISC-186: README contains a per-platform capability matrix (analytics + video constraints).
- [ ] ISC-187: README contains a platform-risk / ToS disclosure section.
- [ ] ISC-188: One canonical readiness doc exists; others carry a "superseded by" note (`rg "superseded" docs/`).
- [ ] ISC-189: `test -f .github/PULL_REQUEST_TEMPLATE.md && test -f CODE_OF_CONDUCT.md`.

#### NS — Safety / recovery (owner-missed)
- [ ] ISC-190: `rg "state export|state import|backup" src/xpst/cli.py` matches (state durability).
- [ ] ISC-191: `pytest -k state_backup_rotates` passes.
- [ ] ISC-192: `rg "require_confirm|readonly" src/xpst/mcp/server.py` matches (agent guardrails) OR documented.
- [ ] ISC-193: `rg "session.*valid|challenge" src/xpst/` shows a session-health probe in `xpst health`.
- [ ] ISC-194: `rg "failures list|failures retry" src/xpst/cli.py` matches (operator recovery loop).
...
(Empty until VERIFY — entries land here with quoted probe output as ISCs pass.)
